...
// Locale map for common currencies — falls back to 'en-US' for unknown codes
const CURRENCY_LOCALES = {
  USD: 'en-US', EUR: 'de-DE', GBP: 'en-GB', INR: 'en-IN', JPY: 'ja-JP',
  CNY: 'zh-CN', AUD: 'en-AU', CAD: 'en-CA', CHF: 'de-CH', KRW: 'ko-KR',
  SGD: 'en-SG', HKD: 'zh-HK', NOK: 'nb-NO', SEK: 'sv-SE', DKK: 'da-DK',
  MXN: 'es-MX', BRL: 'pt-BR', ZAR: 'en-ZA', RUB: 'ru-RU', TRY: 'tr-TR',
  AED: 'ar-AE', SAR: 'ar-SA', NZD: 'en-NZ', THB: 'th-TH', IDR: 'id-ID',
  MYR: 'ms-MY', PHP: 'en-PH', PKR: 'ur-PK', BDT: 'bn-BD', EGP: 'ar-EG',
  NGN: 'en-NG', KES: 'sw-KE', GHS: 'en-GH', TZS: 'sw-TZ', UAH: 'uk-UA',
  PLN: 'pl-PL', CZK: 'cs-CZ', HUF: 'hu-HU', RON: 'ro-RO', ILS: 'he-IL',
};
  ...
  if (budgets?.budgetStats) {
    ...
    if (exceededBudgets.length > 0) {
      insights.push({
        ...
        } been truly exceeded (spent more than the budget)`,
        icon: AlertCircle,
      });
    }
    ...
    if (limitReachedBudgets.length > 0) {
      insights.push({
        ...
        message: `${limitReachedBudgets.length} budget${
          limitReachedBudgets.length > 1 ? "s are" : " is"
        } fully exhausted but not exceeded`,
        icon: Target,
      });
    }
  }
