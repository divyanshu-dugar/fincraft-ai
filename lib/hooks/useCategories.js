...
export const useCategories = (rawEndpoint) => {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      ...
      // Normalize: expense-categories now returns { tree, orphans } instead of flat array
      let cats;
      if (Array.isArray(data)) {
        ...
        for (const p of data.tree) {
          const { subcategories, ...parent } = p;
          cats.push(parent);
          for (const sub of subcategories || []) cats.push(sub);
        }
        for (const o of data.orphans || []) cats.push(o);
      } else {
        cats = [];
      }
      setCategories(cats);
