...
export function ListPageSkeleton({ accentFrom = "from-blue-600", accentVia = "via-indigo-600", accentTo = "to-purple-700" }) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 py-18 animate-pulse">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <div className="h-10 flex-1 max-w-sm bg-slate-800 rounded-xl" />
        ...
        <div className="space-y-3">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="bg-slate-800/60 rounded-2xl border border-slate-700/50 p-4 flex items-center gap-4">
              <div className="w-10 h-10 bg-slate-700 rounded-xl shrink-0" />
              <div className="flex-1 space-y-2">
                <div className="h-4 w-40 bg-slate-700 rounded" />
                <div className="h-3 w-24 bg-slate-700/60 rounded" />
              </div>
              <div className="h-5 w-20 bg-slate-700 rounded hidden sm:block" />
...
export function EditFormSkeleton({ accentColor = "bg-blue-500" }) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 pt-18 animate-pulse">
      {/* Sticky header */}
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-5">
        ...
        <div className="bg-slate-800/60 rounded-2xl border border-slate-700/50 p-6 space-y-4">
          ...
          <div className="flex gap-2">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="h-9 w-16 bg-slate-700/60 rounded-xl" />
            ))}
          </div>
        </div>
        ...
        {/* Form fields */}
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-slate-800/60 rounded-2xl border border-slate-700/50 p-5 space-y-3">
            <div className="h-3 w-20 bg-slate-700 rounded" />
        ...
        {/* Submit button */}
        <div className="h-12 w-full bg-slate-700/60 rounded-2xl" />
      </div>
    </div>
  );
}
...
export function ProfileSkeleton() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 animate-pulse">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-24 pb-16 space-y-6">
        {/* Hero header */}
        <div className="flex items-center gap-5">
          <div className="w-20 h-20 rounded-2xl bg-slate-700" />
          <div className="flex-1 space-y-2">
            <div className="h-6 w-40 bg-slate-700 rounded" />
            <div className="h-4 w-56 bg-slate-700/60 rounded" />
            <div className="h-3 w-32 bg-slate-700/40 rounded" />
          </div>
          <div className="h-7 w-20 bg-slate-700 rounded-full" />
        </div>

        {/* Section cards */}
        {[...Array(3)].map((_, i) => (
          <div key={i} className="bg-slate-800/60 rounded-2xl border border-slate-700/50 p-6 space-y-4">
...
export function GoalDetailSkeleton() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 py-18 px-4 sm:px-6 lg:px-8 animate-pulse">
      <div className="max-w-4xl mx-auto space-y-6">
        {/* Back link */}
        <div className="h-4 w-28 bg-slate-700 rounded mb-6" />

        {/* Header */}
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-2">
            <div className="h-8 w-52 bg-slate-700 rounded-lg" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-800/60 rounded-2xl border border-slate-700/50 p-6 flex items-center justify-center">
            <div className="w-32 h-32 rounded-full border-8 border-slate-700" />
          </div>
          {[...Array(2)].map((_, i) => (
            <div key={i} className="bg-slate-800/60 rounded-2xl border border-slate-700/50 p-6 space-y-3">
        ...
        <div className="bg-slate-800/60 rounded-2xl border border-slate-700/50 p-6 space-y-4">
          ...
          {[...Array(4)].map((_, i) => (
            <div key={i} className="flex items-center gap-3 py-2">
              <div className="w-8 h-8 bg-slate-700 rounded-lg" />
              <div className="flex-1 h-4 bg-slate-700/60 rounded" />
    ...
    <div className="flex flex-col gap-5 h-full justify-center px-4 animate-pulse">
      {[
        ...
        <div key={i} className={`${bubble.align} ${bubble.w} space-y-2`}>
          <div className={`h-10 rounded-2xl ${i % 2 === 0 ? "bg-emerald-500/15" : "bg-slate-800/60"}`} />
        </div>
      ))}
    </div>
  );
...
export function CategoryListSkeleton({ count = 6 }) {
  return (
    <div className="space-y-2 animate-pulse py-4">
      {[...Array(count)].map((_, i) => (
        <div key={i} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-800/40">
