  ...
  return (
    <div className="relative" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className={`flex items-center gap-1 rounded-xl font-black text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-slate-200/60 dark:bg-slate-700/60 transition-all duration-150 select-none group ${
          size === 'lg'
            ? 'text-4xl px-2 py-1'
      ...
      {open && (
        <div className="absolute top-full left-0 mt-2 z-50 min-w-[180px] bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
          ...
          <div className="py-1">
            {currencies.map((c) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left transition-colors hover:bg-slate-200/60 dark:bg-slate-700/60 ${
